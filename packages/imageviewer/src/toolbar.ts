 ...
 */
const IMAGE_TOOLBAR_CLASS = 'jp-ImageToolbar';

const IMAGE_TOOLBAR_LABEL_CLASS = 'jp-ImageToolbar-label';

/**
 * The class name added to a csv toolbar's dropdown element.
 */
const IMAGE_TOOLBAR_DROPDOWN_CLASS = 'jp-ImageToolbar-dropdown';



/**
 * A widget for Image widget toolbars.
 */
export
class ImageToolbar extends Widget {
  /**
   * Construct a new csv table widget.
   */
  constructor(options: ImageToolbar.IOptions) {
    super({ node: Private.createNode(options.selected) });
    this.addClass(Image_TOOLBAR_CLASS);
  }
...
/**
 * A namespace for `ImageToolbar` statics.
 */
export
namespace ImageToolbar {
  /**
   * The instantiation options for a Image toolbar.
   */
  export
  interface IOptions {
    /**
...
namespace Private {
  ...
  function createNode(selected: string): HTMLElement {
    ...
    label.className = IMAGE_TOOLBAR_LABEL_CLASS;
    ...
    node.classList.add(IMAGE_TOOLBAR_DROPDOWN_CLASS);
